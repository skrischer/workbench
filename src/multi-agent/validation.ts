...
import type { SpawnConfig, AgentMessage, AgentRole } from '../types/agent.js';

const VALID_ROLES: AgentRole[] = ['planner', 'worker', 'reviewer', 'custom'];
const VALID_MESSAGE_TYPES = ['task', 'result', 'status', 'error'] as const;
 ...
 */
export function validateSpawnConfig(config: unknown): asserts config is SpawnConfig {
  ...
  if (!VALID_ROLES.includes(cfg.role as AgentRole)) {
    throw new Error(
      `SpawnConfig role must be one of: ${VALID_ROLES.join(', ')} (got: ${cfg.role})`
    );
  }
...
export function validateAgentMessage(message: unknown): asserts message is AgentMessage {
  ...
  if (!VALID_MESSAGE_TYPES.includes(msg.type as any)) {
    throw new Error(
      `AgentMessage type must be one of: ${VALID_MESSAGE_TYPES.join(', ')} (got: ${msg.type})`
    );
  }
