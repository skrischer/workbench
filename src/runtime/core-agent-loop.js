...
export class CoreAgentLoop {
    ...
    config;
    eventBus;
    constructor(anthropicClient, sessionStorage, toolRegistry, config, eventBus) {
    ...
    async run(prompt) {
        ...
        try {
            ...
            while (step < this.config.maxSteps) {
                ...
                // c. Send to LLM
                const response = await this.anthropicClient.sendMessage(llmMessages, tools.length > 0 ? tools : undefined, { system: this.config.systemPrompt });
                // Track token usage
