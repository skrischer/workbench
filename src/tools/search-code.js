...
export class SearchCodeTool extends BaseTool {
    name = 'search_code';
    description = 'Search for code constructs (functions, classes, interfaces, types) by name using regex heuristics. Returns matches with file path, line number, and context.';
    inputSchema = {
        type: 'object',
        properties: {
            query: {
        ...
        try {
            ...
            for await (const entry of walkDirectory(searchPath, { depth: 10 })) {
                ...
                if (allowedExtensions.length > 0) {
                    const hasAllowedExt = allowedExtensions.some((ext) => entry.name.endsWith(ext));
                    if (!hasAllowedExt)
                        continue;
                }
     ...
     */
    searchInFile(content, query, type) {
        ...
        if (type === 'all' || type === 'function') {
            ...
            patterns.push(new RegExp(`\\bfunction\\s+${escapedQuery}\\b`, 'i'), new RegExp(`\\b${escapedQuery}\\s*[:=]\\s*function\\b`, 'i'), new RegExp(`\\b${escapedQuery}\\s*[:=]\\s*\\([^)]*\\)\\s*=>`, 'i'), new RegExp(`\\bconst\\s+${escapedQuery}\\s*=\\s*\\([^)]*\\)\\s*=>`, 'i'), new RegExp(`\\bdef\\s+${escapedQuery}\\b`, 'i'), // Python
            new RegExp(`\\bfn\\s+${escapedQuery}\\b`, 'i'), // Rust
            new RegExp(`\\bfunc\\s+${escapedQuery}\\b`, 'i'));
        }
        if (type === 'all' || type === 'class') {
            // Match class declarations
            patterns.push(new RegExp(`\\bclass\\s+${escapedQuery}\\b`, 'i'));
        }
        if (type === 'all' || type === 'interface') {
            // Match interface declarations
            patterns.push(new RegExp(`\\binterface\\s+${escapedQuery}\\b`, 'i'));
        }
        if (type === 'all' || type === 'type') {
            // Match type declarations
            patterns.push(new RegExp(`\\btype\\s+${escapedQuery}\\b`, 'i'));
        }
        if (type === 'all' || type === 'export') {
            // Match export statements
            patterns.push(new RegExp(`\\bexport\\s+.*\\b${escapedQuery}\\b`, 'i'));
        }
        if (type === 'all' || type === 'import') {
            // Match import statements
            patterns.push(new RegExp(`\\bimport\\s+.*\\b${escapedQuery}\\b`, 'i'), new RegExp(`\\bfrom\\s+['"].*${escapedQuery}.*['"]`, 'i'));
        }
        // Search each line
