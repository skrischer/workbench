...
export class GrepTool extends BaseTool {
    name = 'grep';
    description = 'Search files recursively using regex patterns. Returns matches with line numbers and optional context.';
    inputSchema = {
        type: 'object',
        properties: {
            pattern: {
     ...
     */
    async searchFile(filePath, regex, contextLines) {
        const matches = [];
        try {
     ...
     */
    formatOutput(matches, basePath, wasTruncated) {
        if (matches.length === 0) {
