...
export class LanceDBMemoryStore {
     ...
     */
    constructor(dbPath, tableName = 'memories', embeddingProvider) {
    ...
    async init() {
        ...
        try {
            ...
            throw new Error(`Failed to initialize LanceDB store: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    async listByType(type) {
        ...
        const results = await this.table
            ...
            .where(`type = '${type}'`)
            .toArray();
        return results.map((record) => this.recordToEntry(record));
    }
    /**
